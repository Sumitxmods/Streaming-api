#!/usr/bin/env python3
"""
💣 SUMIT X MODS — ATOMIC BOMB v6.2 [TG ADDED]
- YouTube: 🔥 Spoof Pool + 4 Fallback Methods
- Telegram: 📱 Direct MP4 + yt-dlp extraction
...
def extract_telegram(url):
    """Telegram video — direct MP4 extraction"""
    print(f"[TG] Extracting: {url}")
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if info.get("url"):
                print(f"[TG] ✅ yt-dlp direct")
                return {"url": info["url"], "expiry": expiry, "source": "telegram"}
            for f in info.get("formats", []):
                if f.get("url"):
                    print(f"[TG] ✅ yt-dlp format")
                    return {"url": f["url"], "expiry": expiry, "source": "telegram"}
    except Exception as e:
        print(f"[TG] yt-dlp: {str(e)[:60]}")
    ...
    try:
        ...
        if match:
            print(f"[TG] ✅ og:video")
            return {"url": match.group(1), "expiry": expiry, "source": "telegram_og"}
        
        # Find video source
        match = re.search(r'<source[^>]+src="([^"]+\.mp4[^"]*)"', html)
        if match:
            print(f"[TG] ✅ source tag")
            return {"url": match.group(1), "expiry": expiry, "source": "telegram_source"}
        
        # Find any mp4 URL
        match = re.search(r'["\']([^"\']+\.mp4[^"\']*)["\']', html)
        if match:
            print(f"[TG] ✅ mp4 found")
            return {"url": match.group(1), "expiry": expiry, "source": "telegram_mp4"}
            
    except Exception as e:
        print(f"[TG] scrape: {str(e)[:60]}")
...
def extract_youtube(url):
    ...
    for instance in ["https://inv.nadeko.net", "https://vid.puffyan.us", "https://invidious.flokinet.to"]:
        try:
            resp = requests.get(f"{instance}/api/v1/videos/{video_id}", timeout=10, headers={"User-Agent": selected["ua"]})
    ...
    for instance in ["https://pipedapi.kavin.rocks", "https://pipedapi.tokhmi.xyz"]:
        try:
            resp = requests.get(f"{instance}/streams/{video_id}", timeout=10, headers={"User-Agent": selected["ua"]})
            if resp.status_code == 200:
    ...
    try:
        r = requests.post("https://app.ytdown.to/proxy.php",
            ...
            headers={"Content-Type":"application/x-www-form-urlencoded","User-Agent":selected["ua"],"Origin":"https://app.ytdown.to","Referer":"https://app.ytdown.to/"}, timeout=20)
        d = r.json()
        if d.get("links"):
            best = d["links"][0]
            for l in d["links"]:
...
def extract_fresh(original_url):
    ...
    if "t.me" in domain or "telegram" in domain:
        return extract_telegram(original_url)
...
def home():
    return jsonify({
        "status": "💣 Atomic Bomb v6.2 [TG ADDED]",
        "app": "Sumit x mods",
        "platforms": "YouTube | Telegram | Pinterest | Instagram | Direct"
    })
...
def add_video():
    ...
    domain = urlparse(original_url).netloc.lower()
    if "t.me" in domain or "telegram" in domain: st = "telegram"
    elif any(x in domain for x in ["youtube.com","youtu.be"]): st = "youtube"
