...
- YouTube: 🔥 SPOOF POOL (iOS, Android, Web Embedded)
...
def extract_youtube(url):
    """YouTube with Spoof Pool — iOS + Web Embedded clients"""
    video_id = None
    parsed = urlparse(url)
    ...
    print(f"[YT-SPOOF] Extracting: {video_id}")
    ...
    try:
        ...
        ydl_opts = {
            ...
            'extractor_args': {
                'youtube': {
                    'client': ['ios', 'web_embedded', 'android_embed'],
                }
            },
            'quiet': True, 'no_warnings': True, 'skip_download': True,
            'nocheckcertificate': True, 'geo_bypass': True, 'noplaylist': True,
            'socket_timeout': 20,
            'http_headers': {
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # Priority: progressive formats (video + audio merged)
            if info.get('formats'):
                progressive = [
                ...
                if progressive:
                    streaming_url = progressive[-1].get('url')
            
            if not streaming_url:
                streaming_url = info.get('url')

            if streaming_url:
                print(f"[YT-SPOOF] ✅ {video_id} via {selected['ip']}")
                return {"url": streaming_url, "expiry": expiry, "source": "yt_spoof"}
    
    except Exception as e:
        print(f"[YT-SPOOF] Error: {e}")
...
def extract_fresh(original_url):
    ...
    if any(x in domain for x in ["youtube.com", "youtu.be"]):
        return extract_youtube(original_url)  # 🔥 SPOOF POOL
